...
async function initConverter(accounts, activate) {
    ...
    let converter = await TdrConverter.new(tokenAddress, converterExtensionsAddress, 0, connectorTokenAddress, 250000);
...
contract('TdrConverter', (accounts) => {
    before(async () => {
        let token = await SmartToken.new('Token1', 'TKN1', 2);
        let formula = await TdrFormula.new();
        let gasPriceLimit = await TdrGasPriceLimit.new(gasPrice);
        let quickConverter = await TdrQuickConverter.new();
        let converterExtensions = await TdrConverterExtensions.new(formula.address, gasPriceLimit.address, quickConverter.address);
    ...
    it('verifies the converter data after construction', async () => {
        let converter = await TdrConverter.new(tokenAddress, converterExtensionsAddress, 0, '0x0', 0);
    ...
    it('should throw when attempting to construct a converter with no token', async () => {
        try {
            await TdrConverter.new('0x0', converterExtensionsAddress, 0, '0x0', 0);
    ...
    it('should throw when attempting to construct a converter with no converter extensions', async () => {
        try {
            await TdrConverter.new(tokenAddress, '0x0', 0, '0x0', 0);
            assert(false, "didn't throw");
        }
        catch (error) {
            return utils.ensureException(error);
    ...
    it('should throw when attempting to construct a converter with invalid max fee', async () => {
        try {
            await TdrConverter.new(tokenAddress, converterExtensionsAddress, 1000000000, '0x0', 0);
            assert(false, "didn't throw");
        }
        catch (error) {
            return utils.ensureException(error);
    ...
    it('verifies the first connector when provided at construction time', async () => {
        let converter = await TdrConverter.new(tokenAddress, converterExtensionsAddress, 0, connectorTokenAddress, 200000);
    ...
    it('should throw when attempting to construct a converter with a connector with invalid weight', async () => {
        try {
            await TdrConverter.new(tokenAddress, converterExtensionsAddress, 0, connectorTokenAddress, 1000001);
            assert(false, "didn't throw");
        }
        catch (error) {
            return utils.ensureException(error);
    ...
    it('verifies the owner can update the fee', async () => {
        let converter = await TdrConverter.new(tokenAddress, converterExtensionsAddress, 200000, '0x0', 0);
    ...
    it('should throw when attempting to add a connector when the converter is active', async () => {
        let token = await SmartToken.new('Token1', 'TKN1', 2);
        let converter = await TdrConverter.new(token.address, converterExtensionsAddress, 0, '0x0', 0);
